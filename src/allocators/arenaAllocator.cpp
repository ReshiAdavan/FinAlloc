...
void* ArenaAllocator::allocate(std::size_t bytes, std::size_t alignment) {
    ...
    if (alignment < kMinAlign) alignment = kMinAlign;
    if ((alignment & (alignment - 1)) != 0) alignment = next_pow2(alignment);
...
void ArenaAllocator::release() {
    ...
    if (group_) {
        for (auto& c : chunks_) group_->release(std::move(c));
    } else {
        for (auto& c : chunks_) osFreeChunk_(c);
    }
    chunks_.clear();
    totalBytes_ = 0;
    nextChunkBytes_ = std::max<std::size_t>(opts_.initial_chunk_size, std::size_t{4096});
...
void* ArenaAllocator::allocateSlow_(std::size_t size, std::size_t alignment) {
    ...
    if (next < worst) next = worst;
    if (next < opts_.initial_chunk_size) next = opts_.initial_chunk_size;
    if (next > opts_.max_chunk_size)     next = opts_.max_chunk_size;
...
bool ArenaAllocator::tryAllocFromChunk_(ArenaChunk& c, std::size_t userSize, std::size_t alignment, void** out) {
    ...
    // base = c.base
    // cur  = base + c.offset
    // hdr  = align_up(cur, alignof(max_align_t))
    // hdr_end = hdr + sizeof(BlockHeader)
    // user = align_up(hdr_end + pre, alignment)
    // end  = user + userSize + post
    // ensure end <= base + c.size
    ...
    if (end > base + c.size) return false;
...
void ArenaAllocator::writeCanaries_(unsigned char* user, std::size_t size, std::size_t pre, std::size_t post) {
    if (pre  > 0) std::memset(user - pre,         opts_.canary_byte, pre);
    if (post > 0) std::memset(user + size,        opts_.canary_byte, post);
}

void ArenaAllocator::maybeJournal_(std::size_t size, std::size_t alignment) {
    if (!journalOn_) return;
    if (size < opts_.journal_threshold_bytes) return;
...
static inline std::size_t pick_index(std::size_t minBytes) {
    for (std::size_t i = 0; i < BIN_COUNT; ++i) {
        if (class_bytes(i) >= minBytes) return i;
    }
    return BIN_COUNT - 1;
}
} // namespace
...
ArenaGroup::Chunk ArenaGroup::acquire(std::size_t minBytes, bool guards, bool preferHuge) {
    ...
    if (idx >= bins_.size()) bins_.resize(BIN_COUNT);
...
void ArenaGroup::release(Chunk&& chunk) {
    if (!chunk.base || chunk.size == 0) return;
    std::lock_guard<std::mutex> lock(mtx_);
