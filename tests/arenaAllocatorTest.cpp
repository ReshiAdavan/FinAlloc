...
static void test_basic_construct_and_alignment()
{
    ...
    for (int i = 0; i < 2000; ++i)
    {
        ...
        if (!p)
        {
            std::cerr << "construct returned null\n";
            std::abort();
        }
    ...
    for (std::size_t align : {8ull, 64ull, 256ull, 4096ull})
    {
        ...
        if (!p)
        {
            std::cerr << "allocate returned null (align=" << align << ")\n";
            std::abort();
        }
...
static void test_growth_and_reset()
{
    ...
    for (int i = 0; i < 10; ++i)
    {
        ...
        if (!p)
        {
            std::cerr << "allocate returned null during growth\n";
            std::abort();
        }
    }
    ...
    for (int i = 0; i < 1000; ++i)
    {
        ...
        if (!p)
        {
            std::cerr << "allocate returned null after reset\n";
            std::abort();
        }
    }
...
static void test_arena_group_recycler()
{
    ...
    // direct acquire/release
    {
        ...
        if (!c1.base || c1.size < 64 * 1024)
        {
            std::cerr << "group acquire failed\n";
            std::abort();
        }
    ...
    }
    {
        ...
        if (!c2.base || c2.size < 32 * 1024)
        {
            std::cerr << "group re-acquire failed\n";
            std::abort();
        }
