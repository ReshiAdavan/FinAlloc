...
// mirror alignUp used by the allocator so we know how many bytes to inspect
...
int main() {
    ...
    // -------- Section A: Zeroing + Poison verification on re-use --------
    {
    ...
    // -------- Section B: Quarantine behavior (simple deterministic cases) --------
    {
        ...
        // Case B1: capacity=4, quarantine=4 -> freelist empty after freeing all, next alloc should fail
        {
            ...
            for (int i = 0; i < 4; ++i) pool.deallocate(a[i]);

            void* should_be_null = pool.allocate();
            require(should_be_null == nullptr, "B1: allocate should fail due to full quarantine");
        }

        // Case B2: capacity=5, quarantine=4 -> freeing 5th will flush 1 back; next alloc succeeds
        {
            PoolOptions o = PoolOptions::MinimalOverhead();
            ...
            for (int i = 0; i < 5; ++i) pool.deallocate(a[i]); // 5th pushes one back to freelist
    ...
    // -------- Section C: Metrics sanity (multi-thread) --------
    {
